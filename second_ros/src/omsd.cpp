...
tf::TransformListener listener1;
  ...
  */
  ...
  target_pose1.position.x +=bk*np[0];
  target_pose1.position.y +=bk*np[1];
  target_pose1.position.z +=bk*np[2];
...
geometry_msgs::PoseStamped target_pose22;
  ...
  //target_pose2.orientation.z=q.z;
  //target_pose2.orientation.w=q.w;
  target_pose2.position.x +=bk*np[0];
  target_pose2.position.y +=bk*np[1];
  target_pose2.position.z +=bk*np[2];
...
sub = node_handle.subscribe("chatter", 1000, chatterCallback);
  ...
  //target_pose2.orientation.z=q.z;
  //target_pose2.orientation.w=q.w;
  target_pose3.position.x +=bk*np[0];
  target_pose3.position.y +=bk*np[1];
  target_pose3.position.z +=bk*np[2];
  ...
  //ここまでは，三点の座標から，平面の方程式を算出している．
    ...
    double lengths=pow((A*A)+(B*B)+(C*C),0.5);
    A=A/lengths;
    B=B/lengths;
    C=C/lengths;
  ...
  */
  ...
  static tf::TransformBroadcaster br2;
    ...
    static tf::TransformBroadcaster br3;
            ...
            transform3.setOrigin(tf::Vector3(A,B,C));
            transform3.setRotation(tf::Quaternion(q.x,q.y,q.z,q.w));
            br3.sendTransform(tf::StampedTransform(transform3, ros::Time::now(), "pr1", "housenn"));     


    static tf::TransformBroadcaster br50;
            tf::Transform transform50;
            transform50.setOrigin(tf::Vector3(-A,-B,-C));
            transform50.setRotation(tf::Quaternion(q.x,q.y,q.z,q.w));
            br50.sendTransform(tf::StampedTransform(transform50, ros::Time::now(), "pr1", "housenn1"));         
    
    static tf::TransformBroadcaster br1;
            tf::Transform transform1;
            transform1.setOrigin(tf::Vector3(b[0],b[1],b[2]));
            transform1.setRotation(tf::Quaternion(q.x,q.y,q.z,q.w));
            br1.sendTransform(tf::StampedTransform(transform1, ros::Time::now(), "base_link", "pr2"));

    static tf::TransformBroadcaster br4;
            tf::Transform transform4;
            transform4.setOrigin(tf::Vector3(c[0],c[1],c[2]));
            transform4.setRotation(tf::Quaternion(q.x,q.y,q.z,q.w));
            br4.sendTransform(tf::StampedTransform(transform4, ros::Time::now(), "base_link", "pr3"));        
